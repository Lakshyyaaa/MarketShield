...
OFFICIAL_FINANCE_DOMAINS = {
    ...
    "5paisa.com", "paytmmoney.com", "dhan.co", "scores.gov.in", "smartodr.in",
    "cdslindia.com", "nsdl.co.in", "scores.sebi.gov.in",
}
BRAND_KEYWORDS = [
    "sebi", "nse", "bse", "rbi", "zerodha", "groww", "angelone", "upstox",
    "icicidirect", "hdfcsec", "kotak", "sharekhan", "motilaloswal", "5paisa",
    "paytmmoney", "dhan", "scores", "smartodr", "cdsl", "nsdl",
]
...
CONTENT_PATTERNS = [
    ("guaranteed_return", TIER_HIGH, 35, [
        ...
        r"गारंटी", r"मुनाफा", r"पक्का\s+मुनाफा", r"निश्चित\s+रिटर्न", r"\bjackpot\s+calls?\b", r"\bsure\s+shot\s+call\b",
    ]),
    ...
    ("payment_solicitation", TIER_HIGH, 28, [
        ...
        r"\bactivation\s+fee\b", r"\bregistration\s+fee\b", r"\bmembership\s+(?:fee|charges?|rs\.?|₹|inr)\b",
        r"फीस", r"पैसे\s+(भेजें|जमा|ट्रांसफर)",
    ]),
    ("urgency", TIER_MEDIUM, 18, [
        ...
        r"सिर्फ\s+\d+\s+(दिन|घंटे|सीट)", r"जल्दी\s+करें",
    ]),
    ("fomo", TIER_MEDIUM, 15, [
...
PROFIT_WORD_RE = re.compile(r"\b(returns?|profit|gains?|increase|double|triple|multibagger|jackpot|मुनाफा|फायदा|कमाई)\b", re.I)
...
DAILY_RE = re.compile(r"\b(daily|per\s+day|each\s+day|प्रतिदिन|रोज)\b", re.I)
MONTHLY_RE = re.compile(r"\b(monthly|per\s+month|each\s+month|महीने|प्रति\s+माह)\b", re.I)
PERIOD_DAYS_RE = re.compile(r"(?:in\s+(\d{1,3})\s+days?|(\d{1,3})\s+दिन\s+में)", re.I)
DOUBLE_RE = re.compile(r"\b(double(?:s|d)?|पैसा\s*(?:डबल|दोगुना)|paisa\s*double|2x)\b", re.I)
TRIPLE_RE = re.compile(r"\b(triple(?:s|d)?|तीन\s*गुना|3x)\b", re.I)
IPO_ASBA_RE = re.compile(r"(?:\bipo\b.{0,60}\b(?:allotment|apply|application|quota|shares?)\b|\b(?:allotment|apply|application|quota|shares?)\b.{0,60}\bipo\b)", re.I | re.S)
...
def score_text(text):
    ...
    # Also detect explicit double / triple / 2x returns phrases in English or Hindi
    for dm in DOUBLE_RE.finditer(text):
        naive_multiplier_hits.append({"value": 2, "weight": 30})
        polarity = reasoning.classify_polarity(text, doc, dm.start(), dm.end())
        if polarity == reasoning.POSITIVE_RISK:
            spans.append((dm.start(), dm.end(), "multiplier"))
            evidence_by_category.setdefault("multiplier", []).append(text[dm.start():dm.end()])
            all_spans.append({
                "start": dm.start(), "end": dm.end(), "text": text[dm.start():dm.end()], "category": "multiplier",
                "polarity": reasoning.POSITIVE_RISK, "tag": None,
                "explanation": f"Counted as risk evidence: {SIGNAL_WHY.get('multiplier', '')}",
            })
            multiplier_hits.append({"value": 2, "weight": 30})

    for tm in TRIPLE_RE.finditer(text):
        naive_multiplier_hits.append({"value": 3, "weight": 35})
        polarity = reasoning.classify_polarity(text, doc, tm.start(), tm.end())
        if polarity == reasoning.POSITIVE_RISK:
            spans.append((tm.start(), tm.end(), "multiplier"))
            evidence_by_category.setdefault("multiplier", []).append(text[tm.start():tm.end()])
            all_spans.append({
                "start": tm.start(), "end": tm.end(), "text": text[tm.start():tm.end()], "category": "multiplier",
                "polarity": reasoning.POSITIVE_RISK, "tag": None,
                "explanation": f"Counted as risk evidence: {SIGNAL_WHY.get('multiplier', '')}",
            })
            multiplier_hits.append({"value": 3, "weight": 35})
    ...
    if auto_critical:
        # High-severity signals cannot be neutralized by boilerplate disclaimers; score reflects natural accumulated evidence
        score = min(raw_score, 100)
        protective_effect = 0
    else:
        protective_effect = min(len(protective_matches) * PROTECTIVE_UNIT_EFFECT, PROTECTIVE_MAX_EFFECT)
        score = min(max(raw_score - protective_effect, 0), 100)
...
def fuse_all(buckets):
    """Calculates final scam score strictly based on the pillars actively involved in the detection.
    Uninvolved pillars (not present or not relevant to the threat vector) do not dilute the score."""
    active_parts = [(name, b["score"], BUCKET_WEIGHTS[name]) for name, b in buckets.items() if b.get("available")]
    if not active_parts:
        return 0, 40

    # 1. Base score calculated as weighted average over ONLY the actively involved pillars
    total_active_weight = sum(w for _, _, w in active_parts)
    weighted = sum(s * w for _, s, w in active_parts) / total_active_weight if total_active_weight > 0 else 0

    # 2. Corroborating signals: when multiple distinct risk dimensions are elevated (>= 15), add dynamic compounding
    elevated = [s for _, s, _ in active_parts if s >= 15]
    corroboration = min(len(elevated) * 6, 24) if len(elevated) >= 2 else 0

    # 3. Peak bucket scaling: ensure an acute high-risk pillar is preserved
    peak = max((s for _, s, _ in active_parts), default=0)

    # 4. Final scam score combines the active pillar weights and compounding without dilution
    overall = max(weighted + corroboration, peak)
    confidence = min(50 + 10 * len(active_parts), 96)
    return round(min(max(overall, 0), 100)), confidence
...
def run_scam_analysis(text=None, url=None, image_bytes=None, image_name=None):
    text = (text or "").strip()
    url = (url or "").strip()
    
    if not text and not url and not image_bytes:
        return {"error": "Provide text, a url, and/or an image to analyze."}
    ...
    if not effective_url and entities.get("urls"):
    ...
    findings = content.get("findings", []) if content.get("available") else []

    # Financial Claim Pillar: actively involved if financial promises, guaranteed returns, percentages, or multipliers were found
    fin_score = content.get("subScores", {}).get("financial_claim", 0) if content.get("available") else 0
    fin_active = bool(content.get("available") and (fin_score > 0 or any(f.get("category") == "financial_claim" for f in findings)))

    # Urgency & Manipulation Pillar: actively involved if urgency keywords, FOMO, or artificial deadlines were found
    urg_score = content.get("subScores", {}).get("urgency_manipulation", 0) if content.get("available") else 0
    urg_active = bool(content.get("available") and (urg_score > 0 or any(f.get("category") == "urgency_manipulation" for f in findings)))

    # Credibility & Intermediary Pillar: actively involved if SEBI credentials, payment requests, advisor identities, or OTP asks were checked
    cred_score = content.get("subScores", {}).get("credibility", 0) if content.get("available") else 0
    cred_active = bool(content.get("available") and (cred_score > 0 or any(f.get("category") == "credibility" for f in findings) or content.get("identity") is not None))

    # Social Signal Pillar: actively involved if Telegram/Instagram channels, WhatsApp numbers, or subscriber ratios were evaluated
    soc_score = social.get("score", 0) if social.get("available") else 0
    soc_active = bool(social.get("available") and (soc_score > 0 or entities.get("socialHandles") or entities.get("telegramUsernames") or entities.get("whatsappNumbers")))

    # URL / Domain Infrastructure Pillar: actively involved if a URL was provided (or auto-detected from text) and audited
    url_score = link.get("score", 0) if link.get("available") else 0
    url_active = bool(link.get("available") and (url_score > 0 or bool(effective_url)))

    # If text was submitted but NO risk indicators were triggered anywhere in the text (e.g. clean educational commentary),
    # mark all text pillars as active with 0 risk so it evaluates as clean.
    if content.get("available") and not (fin_active or urg_active or cred_active):
        fin_active = True
        urg_active = True
        cred_active = True

    buckets = {
        "financial_claim": {"score": fin_score, "available": fin_active},
        "urgency_manipulation": {"score": urg_score, "available": urg_active},
        "credibility": {"score": cred_score, "available": cred_active},
        "social_signal": {"score": soc_score, "available": soc_active},
        "url_risk": {"score": url_score, "available": url_active},
    }
    ...
    result = {
        ...
        "visual_integrity": {
            ...
            "note": "Reported separately from content risk — a manipulated screenshot of a legitimate message and a clean screenshot of a scam ad are different findings.",
        },
        ...
        "social_handles": entities.get("socialHandles", []),
...
def get_demo_seed_cases():
    return {"demoMode": DEMO_MODE, "cases": SEED_CASES}


if FLASK_AVAILABLE and app is not None:
    @app.route("/api/demo-cases", methods=["GET"])
    def api_demo_cases():
        return jsonify(get_demo_seed_cases())
    ...
    def api_history():
        return jsonify(get_scam_history())
